...
const VoteContextProvider = ({ children }) => {
  ...
  const createUserVote = async (userGetVoteId,createVote) => {
    const token = localStorage.getItem("user");
      await axios
        .post(
          `http://localhost:3333/user/getvote/${userGetVoteId}`,
          createVote,
          {
  ...
  const GetVoteResult = (userId) => {
    useEffect( ()=>{
      const token = localStorage.getItem("user");
       axios
        .get(
          `http://localhost:3333/user/votescore/${userId}`,
          {
            headers: { authorization: `Bearer ${token}` },
          }
        )
  ...
  const GetVoteUserResult = () => {
    useEffect( ()=>{
      const token = localStorage.getItem("user");
       axios
        .get(
          `http://localhost:3333/user/votescore`,
          {
            headers: { authorization: `Bearer ${token}` },
          }
        )
 ...
 const UpdateUserLevel = async(userId, sumVote) => {
  ...
  return (
    <voteContext.Provider value={{ handleVoteInput, createVote,createUserVote,GetVoteResult,voteResult,GetVoteUserResult,UpdateUserLevel,userLevel }}>
      {children}
